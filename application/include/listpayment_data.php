...
$myloan = mysqli_query($link, "SELECT amount FROM loan_info WHERE borrower = '$accountid'") or die (mysqli_error($link));
$loanfetch = mysqli_fetch_array($myloan);
...
?>    
				...
				<td><?php echo 'Flexible('.$currency.number_format($loanfetch['amount'],2,".",",").')'; ?></td>
                <td><?php echo $currency.number_format($balance,2,".",","); ?></td>
				<td><?php echo $currency.number_format($amount_to_pay,2,".",","); ?></td>
				<td><?php echo $currency.number_format($row['penalty_amount'],2,".",","); ?></td>
				<td><?php echo $row['months_delayed']; ?></td>
